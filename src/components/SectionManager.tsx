...
const SectionManager = () => {
    ...
    try {
      ...
      if (data.success) {
        const sectionsArray = (Array.isArray(data.data) ? data.data : Object.values(data.data))
          ...
          .filter((s: SectionContent) => (s.id || '').toLowerCase().trim() !== 'whychooseus');
  ...
  const getSectionDescription = (sectionId: string) => {
    switch (sectionId) {
      case 'header': return 'Logo, navigation menu, and language selector settings';
      case 'hero': return 'Main hero section with title, background video, and hotspots';
      case 'whoAmI': return 'About section with company profile and features';
      case 'whyChooseUs': return 'Why choose us with key benefits';
